/* this program is to test the so called heterogeneous Memory Management (HMM) which is simplier than cudaMallocManage(), but the test result in this case at 09/22/2024 shows that HMM does not work, jsut as it was delcared that it is still not bug-free */
...
int main(int argc, char *argv[])
{
	...
	pa = (double*)calloc(Tsize/sizeof(double), sizeof(double));
	pb = (double*)calloc(Tsize/sizeof(double), sizeof(double));
	pc = (double*)calloc(Tsize/sizeof(double), sizeof(double));
