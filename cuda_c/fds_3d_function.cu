...
__global__ void cube_minus(double *a, double *b, double *c, int N_i, int N_j, int N_k, int M) { 
	int index_i = blockIdx.x*blockDim.x + threadIdx.x; 
	int index_j = blockIdx.y*blockDim.y + threadIdx.y;
	int index_k = blockIdx.z*blockDim.z + threadIdx.z;
...
__global__ void cube_product(double *a, double *b, double *c, int N_i, int N_j, int N_k, int M) { 
	int index_i = blockIdx.x*blockDim.x + threadIdx.x; 
	int index_j = blockIdx.y*blockDim.y + threadIdx.y;
	int index_k = blockIdx.z*blockDim.z + threadIdx.z;
...
{ 
	...
	switch ( Operator ){
		case '+':
        		cube_add<<<grid, block>>>( a_d, b_d, c_d, N_i, N_j, N_k, M); 
			break;
		case '-':
        		cube_minus<<<grid, block>>>( a_d, b_d, c_d, N_i, N_j, N_k, M); 
			break;
		case '*':
        		cube_product<<<grid, block>>>( a_d, b_d, c_d, N_i, N_j, N_k, M); 
			break;
	}
